...
export class PacaAPIClient {
  ...
  async listProjects(page: number = 1, pageSize: number = 50): Promise<Project[]> {
  ...
  async listTasks(
    ...
    if (options.pageSize !== undefined) params.push(`page_size=${options.pageSize}`);
    if (options.sprintId !== undefined) params.push(`sprint_id=${options.sprintId}`);
    if (options.statusId !== undefined) params.push(`status_id=${options.statusId}`);
    if (options.assigneeId !== undefined) params.push(`assignee_id=${options.assigneeId}`);
    if (options.taskTypeIds && options.taskTypeIds.length > 0) params.push(`task_type_ids=${options.taskTypeIds.join(",")}`);
    if (options.parentTaskId !== undefined) params.push(`parent_task_id=${options.parentTaskId}`);

    const queryString = params.length > 0 ? `?${params.join("&")}` : "";
    const response = await this.get(`/api/v1/projects/${projectId}/tasks${queryString}`);
    if (Array.isArray(response)) {
      return response;
    }
  ...
  async createTask(input: CreateTaskInput): Promise<Task> {
    ...
    if (input.task_type_id !== undefined) body.task_type_id = input.task_type_id;
    ...
    if (input.parent_task_id !== undefined) body.parent_task_id = input.parent_task_id;
  ...
  async listDocuments(projectId: string, folderId?: string): Promise<Document[]> {
    const queryString = folderId ? `?folder_id=${folderId}` : "";
    const response = await this.get(`/api/v1/projects/${projectId}/docs${queryString}`);
    if (Array.isArray(response)) {
      return response;
    }
