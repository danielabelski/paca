...
import {
  ...
  type BDDScenario,
  type CreateBDDScenarioInput,
  type UpdateBDDScenarioInput,
  type PullRequest,
  type TaskBranch,
  type LinkPRInput,
...
export class PacaAPITaskExtendedClient {
  ...
  async listBDDScenarios(
    ...
    return response.items || response.scenarios || response.bddScenarios || response.data || [];
  }
  ...
  async listTaskPRs(
    ...
    return response.items || response.pullRequests || response.prs || response.data || [];
  }
