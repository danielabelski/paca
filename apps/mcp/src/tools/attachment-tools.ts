...
import { PacaAPITaskExtendedClient, PacaAPIViewsClient } from "../api/index.js";
  ...
  return [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        required: ["projectId", "taskId", "uploadId", "key", "fileName", "fileSize", "mimeType"],
      },
    },
    {
...
export async function handleAttachmentTool(
  ...
  switch (toolName) {
    case "list_task_attachments": {
      ...
      const attachments = await viewsClient.listTaskAttachments(projectId, taskId);
    ...
    case "initiate_attachment_upload": {
      const { projectId, taskId, fileName, fileSize, mimeType } = InitiateAttachmentUploadSchema.parse(args);
    ...
    case "complete_attachment_upload": {
      const { projectId, taskId, uploadId, key, fileName, fileSize, mimeType } = CompleteAttachmentUploadSchema.parse(args);
    ...
    case "get_attachment_download_url": {
      const { projectId, taskId, attachmentId } = GetAttachmentDownloadURLSchema.parse(args);
    ...
    case "delete_task_attachment": {
      const { projectId, taskId, attachmentId } = DeleteTaskAttachmentSchema.parse(args);
