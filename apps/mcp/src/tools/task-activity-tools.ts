...
import { PacaAPITaskExtendedClient } from "../api/index.js";
import { z } from "zod";
...
export async function handleTaskActivityTool(
  ...
  switch (toolName) {
    ...
    case "add_task_comment": {
      ...
      const comment = await client.addTaskComment(projectId, taskId, { text: content });
      return {
        content: [
          {
            type: "text",
    ...
    case "update_task_comment": {
      const { projectId, taskId, commentId, content } = UpdateTaskCommentSchema.parse(args);
      const comment = await client.updateTaskComment(projectId, taskId, commentId, {
        text: content,
      });
      return {
        content: [
    ...
    case "delete_task_comment": {
      const { projectId, taskId, commentId } = DeleteTaskCommentSchema.parse(args);
    ...
    case "link_pr_to_task": {
      const { projectId, taskId, prId, repositoryName, owner } = LinkPRToTaskSchema.parse(args);
    ...
    case "create_branch_for_task": {
      const { projectId, taskId, branchName, baseBranch } = CreateBranchForTaskSchema.parse(args);
