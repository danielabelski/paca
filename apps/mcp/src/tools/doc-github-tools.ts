...
import { PacaAPIDocClient, PacaAPIGitHubClient, PacaAPITaskExtendedClient } from "../api/index.js";
...
export async function handleDocTool(
  ...
  switch (toolName) {
    ...
    case "create_doc_folder": {
      ...
      const folder = await docClient.createFolder(projectId, { name, parent_id: parentId });
      return {
        content: [
          {
            type: "text",
