...
import { PacaAPIExtendedClient } from "../api/index.js";
...
export async function handleProjectMemberTool(
  ...
  switch (toolName) {
    ...
    case "update_project_member_role": {
      const { projectId, userId, roleId } = UpdateProjectMemberRoleSchema.parse(args);
    ...
    case "create_project_role": {
      const { projectId, name, description, permissions } = CreateProjectRoleSchema.parse(args);
    ...
    case "update_project_role": {
      const { projectId, roleId, name, description, permissions } = UpdateProjectRoleSchema.parse(args);
